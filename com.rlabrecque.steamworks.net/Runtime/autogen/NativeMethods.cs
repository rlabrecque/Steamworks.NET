	...
	internal static class NativeMethods {
		...
		private static IntPtr DllImportResolver(string libraryName, System.Reflection.Assembly assembly, DllImportSearchPath? searchPath) {
			...
			if ((libraryName == NativeLibraryName || libraryName == NativeLibrary_SDKEncryptedAppTicket)
				...
				// first chance search, if failed, check the assembly directory for steam natives
				if (!NativeLibrary.TryLoad(x64LibName, assembly, searchPath, out nint lib) || searchPath is DllImportSearchPath.AssemblyDirectory)
				{
					// in case of first chance search failed, build the full path of steam native, include extension name,
					// and try load again, this is for the case when steam native is not in default search path
					...
					if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
						...
						extension = ".so"; // I can't imagine what else platforms Steamworks.NET.AnyCPU will run on, but let's be future proof
					...
					string path = SysPath.Combine(assemblyLocation, SysPath.ChangeExtension(x64LibName, extension));
